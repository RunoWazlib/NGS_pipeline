from ngs_driver import load_config
from config_builder import generate_json_data_paired, generate_json_data_merged
...
def successful_config(tmp_path):
    ...
    fileCopys = [f"cp test_data/x98_query.fasta {tmp_path}/reference.fasta",
                 f"cp test_data/X98-w-Mg_R1_001.fastq.gz {tmp_path}/sample1_R1.fastq.gz",
                 f"cp test_data/X98-w-Mg_R2_001.fastq.gz {tmp_path}/sample1_R2.fastq.gz"]
    
    for command in fileCopys:
...
class TestBasicInitialization:
    def test_paired_config_generation(self, tmp_path):
        ...
        sample_r1 = tmp_path / "sample1_R1.fastq.gz"
        sample_r2 = tmp_path / "sample1_R2.fastq.gz"
        ...
        assert sample_r1 in paired_config["paired-end-mode"]["R1"] # ensure that the R1 file is correctly included in the paired-end config
        assert sample_r2 in paired_config["paired-end-mode"]["R2"] # ensure that the R2 file is correctly included in the paired-end config
    ...
    def test_merged_config_generation(self, tmp_path):
        ...
        assert sample_r1 in merged_config["merged-mode"]["R1"] # ensure that the R1 file is correctly included in the merged config
        assert ["R2"] not in merged_config["merged-mode"] # ensure that the R2 file is not included in the merged config
    ...
    def test_output_dir_generation(self, tmp_path):
        ...
        command = "python ngs_driver.py --config " + str(config_file)
...
class TestBasicBenchmarks:
    def test_benchmark_execution(self, successful_config):
        ...
        command = "python ngs_driver.py --config " + str(successful_config)
        result = subprocess.run(command, shell=True, capture_output=True, text=True)
